...
--------------------------------------------------------------------------------
  ...
  [동작 원리]
  1) 트리거 판정 (1~3초 가상 시간 랜덤 주기로 점검):
     - 예상 대기시간 = 현재 큐 크기 x 평균 처리시간(2초)
     - 임계값(15초) 초과 시 P2P 부하 분산 시작 (큐 8개 이상)
...
--------------------------------------------------------------------------------
  ...
  [--lb-demo 모드 (P2P 시연용)]
  - Master에 --lb-demo 플래그를 주면 초기 40개 작업을 Worker1에 집중 배정
  - Worker1의 큐가 빠르게 쌓여 P2P 부하 분산이 자연스럽게 트리거됨
  - 가상 시간 모델에서 P2P 동작을 시연하기 위한 테스트 모드
  - Worker 처리 루프에 5ms yield를 삽입하여 큐 축적 가능하도록 조정

  [네트워크 지연 모델]
  - Master-Worker 간: 1.0초 (가상, 수신측에서만 적용)
  - Worker-Worker 간(P2P): 0.1초 (같은 로컬 PC이므로 축소)

  [성능 지표 (EC2 실측, --lb-demo 모드)]
  - Worker별 배정: 1970 / 1437 / 1540 / 1223 (lb-demo 편향 포함)
  - 총 SUCCESS: 5,000건, FAIL: 1,170건, REJECT: 0건
  - 평균 대기시간: 2.60초
  - P2P 부하 분산: 6회 (11건 이전)
  - 장애 재할당: 1,170건
  - 전체 수행시간: 4,029.91초 (System Clock)
